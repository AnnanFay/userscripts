        ...
        NeptunesPride.Map = NP2M.wrap(NeptunesPride.Map, function (args, map) {
            ...
            map.drawStars = function () {
                var star, stars = universe.galaxy.stars,
                    ...
                    scanRange   = so && so.player.tech.scanning.value,
                    miniScanSprite = so && {
                        ox: undefined,
                        oy: undefined,
                ...
                for (var i in stars) {
                    ...
                    if (    so && 
                            universe.distance(so.x, so.y, star.x, star.y) <= scanRange && 
                            so !== star) {
                        miniScanSprite.ox = (star.x * map.scale + map.sx) * map.pixelRatio;
                        miniScanSprite.oy = (star.y * map.scale + map.sy) * map.pixelRatio;
