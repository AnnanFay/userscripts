...
(function () {
    ...
    var installed   = false, // to be used so hooks are only installed if a mod registers
        ...
        post_code   = 'NP2M.post_init(Mousetrap, Crux, NeptunesPride);\n',
        mods        = [],
        pre_inits   = [],
        post_inits  = [];
    ...
    NP2M.post_init = function post_init (Mousetrap, Crux, NeptunesPride) {
        var data = {
            "Mousetrap": Mousetrap,
            "Crux": Crux,
  ...
  function voronoiLayerFull(ctx, data, map) {
    var options = {
      ...
      styleModifier: function (style) {
        return d3.lab(style).brighter(1).toString();
      }
    };
    ...
    for (i = 0, l = delaunayLinks.length; i < l; i++) {
      ...
      if (options.borderCondition(source, target)) {
        var interestingPolies = _.filter(voronoiPolygons, function (d) {
          return d && (d.point === source || d.point === target);
        });
        ...
        style = options.styleModifier(style);
