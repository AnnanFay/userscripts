...
(function () {
  ...
  function init () {
    ...
    UPLOAD = store.ensure('upload', 'Upload data', 'true', JSON.parse);
    USERNAME = store.ensure('username', 'Choose something which identifies you.', '');
    GAMESPEED = store.ensure('game-speed', 'One of (N/F/S/U/RT/DM)'); // can we auto detect this?
    PASSPHRASE =  store.ensure('passphrase', 'Choose something that you do not use for anything else. '+
                      'It can be anything. You do not need to remember it, just keep it here. '+
                      'It does not need to contain special characters. It is _not_ secure.', '');
  ...
  var store = {
    ...
    set: function set(name, data) {
      ...
      var v = store.get(name, undefined);
      if (v === undefined) {
        v = ask(name, desc, def);
        if (v !== null) {
          store.set(name, stringHandler ? stringHandler(v) : v);
        }
      }
      return v;
    }
  ...
  function storeRecord (event, eventData) {
    ...
    if (!event.live) {
      ...
      var prevEvents = store.get('event-cache-' + event.name, []);
  ...
  function receiptHandler (e) {
    ...
    if (e.detail && e.detail.hs) {
      ...
      var prevEvents = store.get(storeName, []);
      ...
      store.set(storeName, prevEvents.slice(-cacheSize));
    }
  }
