...
var changes = [
    ...
    change(
        function (anime) {
            ...
            // FIXME: use XPath to get all nodes to check

            for (var i = 0; i < cells.length; i++) {
                var cell_children = cells[i].children;
                if (cell_children.length == 0){
                    cells[i].style.opacity = '0.4';
                }

                for (var j = 0; j < cell_children.length; j++) {
                    var child = cell_children[j];
                    if (child.className == "colored-percent") {
